...
def points(point_pairs):
    ...
    for p, pairs in enumerate(point_pairs):
        ...
        if new_points == []:
            difference = []
            for d_diag, dim_diag in enumerate(dims):
                if (dim_diag[1]-dim_diag[0]) > 0:
                    difference.append(1)
                else:
                    difference.append(-1)
            new_point = list(pairs[0])
            new_points = [new_point]
            final_point = list(pairs[1])
            while new_point != final_point:
                new_point = new_point[:]
                for i, diff_val in enumerate(difference):
                    new_point[i] += diff_val
                new_points.append(new_point)
            '''
            print("DIAGONAL INPUT TIME\n",\
                  "coordinates: ", *pairs, "\n",\
